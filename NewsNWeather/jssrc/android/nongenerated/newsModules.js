 ...
 */
function menuToggle() {
    if (typeof menuLeftPos === 'undefined') menuLeftPos = "90%";
    else if (menuLeftPos === "90%") menuLeftPos = "0%";
    else if (menuLeftPos === "0%") menuLeftPos = "90%";
...
function getNews(category) {
    ...
    try {
        ...
        if (kony.os.deviceInfo().name == "iPhone" || kony.os.deviceInfo().name == "iPhone Simulator" || kony.os.deviceInfo().name == "iPad") kony.application.showLoadingScreen("sknLoading", "Please wait...", constants.LOADING_SCREEN_POSITION_FULL_SCREEN, true, true, {
            shouldShowLabelInBottom: true,
            separatorHeight: 30
        });
        else kony.application.showLoadingScreen("sknLoading", "Please wait...", constants.LOADING_SCREEN_POSITION_FULL_SCREEN, true, true, null);
        ...
        intgService.invokeOperation(configObject.services.news.OPERATION_NAME, {}, {
            "category": category
        }, integSuccessCallback, integFailureCallback);
    } catch (excp) {
	...
	jsonResponse=JSON.parse(response);*/
        ...
        try {
            ...
            for (var i = 0; i < len; i++) {
                ...
                news.lblTitle = {
                    "text": jsonResponse["news_list"][i]["news_item"]["title"]
                };
...
function newsDetails(frmID) {
    ...
    frmBrowser.browserNews.requestURLConfig = {
        "URL": jsonResponse["news_list"][index]["news_item"]["link"],
...
function destroyForm() {
    ...
    frmBrowser.destroy();
    if (frmName === "frmNews") frmNews.show();
    else if (frmName === "frmWeather") frmWeather.show();
}
//var city,zipcode;
/**
 ****************************************************************
...
function weatherIntegSuccessCallback1(response) {
    ...
    //setting news............
    if (kony.os.deviceInfo().name === "android") jsonResponse = response;
    ...
    for (var i = 0; i < len; i++) {
        ...
        kony.print(i + ".\n" + JSON.stringify(jsonResponse["news_list"][i]));
        news.lblTitle = {
            "text": (jsonResponse["news_list"][i]["news_item"]["title"]).substring(0, 25) + ".."
        };
        description = jsonResponse["news_list"][i]["news_item"]["description"];
        kony.print("\n---full description--\n" + description);
        start = description.search('img src="//');
...
function getWeatherForecast() {
    ...
    try {
        ...
        accountsClient.invokeOperation(configObject.services.local.OPERATION_NAME, {}, {
            "lat": lat.toString(),
            "lon": lon.toString()
        }, weatherIntegSuccessCallback, integFailureCallback);
    } catch (excp) {
        kony.application.dismissLoadingScreen();
        kony.print(JSON.stringify(excp));
    }
